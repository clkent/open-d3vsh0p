...
- [x] `nudge-gate-hardening` — Fix false idle continuations seen in the field: `depends` comments yield only their `Phase N` references (trailing notes ignored) and an unresolvable dependency blocks the phase instead of satisfying it; continuation progress is measured as resolution of the unblocked items that justified the continuation, so side commits no longer reset the futile cap
- [x] `group-z-nonblocking-gate` — Roadmap reader's actionable-phase check ignores pending Group Z user-testing checkpoints in dependency phases (non-Group-Z `[HUMAN]` items still block), matching Morgan's rules and the `human-prerequisite-blocking` spec, so idle continuation and `devshop action` see the next phase's work while a checkpoint is left open for the developer
...
- [ ] `remote-control-server` — *(deferred 2026-09-08: built on a branch and closed unmerged — terminal-started sessions already continue from the Claude app, and an always-on phone-driven launcher was judged more exposure than value for now)* `devshop remote` command family: a persistent `claude remote-control` control session in `active-agents/remote/` with a generated CLAUDE.md and narrow permissions, `launch` that starts orchestrator commands detached in named tmux sessions with `--remote-control`, `sessions` and graceful `stop`, and launchd `install`/`remove`/`status` (server runs inside tmux, restart loop with `--continue`)

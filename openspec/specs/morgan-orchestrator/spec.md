...
## Purpose
Replaces the SDK-based isolated agent spawning model in the `run` command with Morgan (Principal Engineer) as a persistent Claude Code CLI session. Morgan reads the roadmap, implements items sequentially with full project context, and can delegate to sub-agents for parallel group work.

## Status
IMPLEMENTED

## Source Files
- `platform/orchestrator/src/commands/run.js` — run command with Morgan CLI spawn
- `templates/agents/principal-engineer/run-prompt.md` — Morgan's orchestration prompt
- `templates/agents/_shared/roadmap-execution-rules.md` — roadmap execution partial
- `templates/agents/_shared/sub-agent-delegation.md` — sub-agent delegation partial
